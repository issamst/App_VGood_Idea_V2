            ...
            if (file != null && file.Length > 0)
            {
                ...
                using (var stream = System.IO.File.Open(filePath, FileMode.Open, FileAccess.Read))
                {
                    using (var reader = ExcelReaderFactory.CreateReader(stream))
                    {
                        ...
                        int nameIndex = -1, teidIndex = -1, managerEmailIndex = -1, plantIdIndex = -1, FullNameIndesx = -1;
                        ...
                        while (reader.Read())
                        {
                            if (!isHeaderSkipped)
                            {
                                ...
                                for (int i = 0; i < reader.FieldCount; i++)
                                {
                                    ...
                                    else if (columnName == "Full Name") FullNameIndesx = i;
                            ...
                            var plantID = await _context.Plant.FirstOrDefaultAsync(d => d.BuildingID == plantName);
                        ...
                        return new OkObjectResult("Departements uploaded successfully.");
                    }
                }
            }
            else
        ...
        public async Task<IActionResult> ExportDepartements()
        {
            try
            {
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    worksheet.Cells[1, 3].Value = "Manager Email";
                    ...
                    for (int i = 0; i < departments.Count; i++)
                    {
                        ...
                        worksheet.Cells[i + 2, 3].Value = department.Manger_Email;
                        worksheet.Cells[i + 2, 4].Value = department.BuildingID?.BuildingID;
                    }
